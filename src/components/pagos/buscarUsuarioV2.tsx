  ...
  return (
    <>
      <Card className={classNames(openState ? 'hidden' : '', 'mx-auto min-w-[18rem] max-w-md')}>
        <Flex>
          <Title>
            {selectValue.display?.trim() === ''
              ? 'Seleccione a un comercio'
              : 'Pagar a ' + selectValue.display}
          </Title>
          <Button
            size='xs'
      ...
      <Card
        className={classNames(
          openState ? '' : 'hidden',
          'mt-2 md:min-w-[442px] min-w-[18rem] max-w-md mx-auto'
        )}>
        ...
        <Card className='mt-2'>
          <List>
            ...
            {data.buscarPerfiles.map((item) => (
              <ListItem
                className={`${
                  selectValue.value === item.id ? 'bg-slate-500/25' : ''
                } cursor-pointer`}
                key={item.id}
