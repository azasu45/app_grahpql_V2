          ...
          <div className='flex flex-wrap justify-start'>
            {previewUrls.map((previewUrl, idx) => (
              <div key={idx} className='w-full p-1.5 object-cover max-h-fit'>
                <Image alt='file uploader preview' src={previewUrl} width={600} height={600} />
              </div>
            ))}
          </div>
        </>
          ...
          <input
            ...
            {...register('file', {
              onChange: (e: ChangeEvent<HTMLInputElement>) => {
                ...
                setPreviewUrls(validFiles.map((validFile) => URL.createObjectURL(validFile))); // we will use this to show the preview of the images
              },
            })}
            accept='.jpg,.png'
          />
